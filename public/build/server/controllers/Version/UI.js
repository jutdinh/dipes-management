...
const {
  UserInterFace,
  UserInterFaceRecord,
} = require("../../models/UserInterFace");
const { translateUnicodeToBlanText } = require("../../functions/auto_value");
const {
  UserInterfaceComponent,
  UserInterfaceComponentRecord,
} = require("../../models/UseInterfaceComponent");
...
class UI extends Controller {
  ...
  getUIs = async (req, res) => {
    ...
    if (success) {
      ...
      const uis = await this.#__ui.getAllUI({
    ...
    if (success) {
      ...
      if (bodyNullCheck.valid) {
        ...
        if (uiNullCheck.valid) {
          ...
          const exestedURL = await this.#__ui.find({
            url: URL,
            version_id: Version.version_id.value(),
          });

          if (!exestedURL) {
            const UI = new UserInterFaceRecord({
              ...ui,
            ...
            if (table_id != undefined && intValidate(table_id)) {
              const table = await this.#__tables.find({
              ...
              if (table) {
                ...
                const GET = {
                  ...
                  fields: fields.map((field) => {
                    return { id: field.id, display_name: field.field_name };
                  }),
                  body: [],
                  status: true,
                  params: [],
                ...
                if (updateBody.length != 0) {
                  if (params.length != 0) {
                    ...
                    context.content =
                      "Bảng khum hợp lệ vì khum có cái khóa chính nào hết trơn";
                  }
                } else {
                  context.success = false;
                  context.content =
                    "Bảng khum hợp lệ vì khum có cái trường nào khác ngoài khóa chính";
                }
              } else {
                context.success = false;
  ...
  removeComponent = async (req, res) => {
    ...
    if (success) {
      ...
      if (component_id != undefined && intValidate(component_id)) {
        const component = this.#__uic.find({
