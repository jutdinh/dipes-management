...
class VersionsController extends Controller {
  ...
  getOneVersion = async (req, res) => {
    ...
    if (verified) {
      ...
      if (account) {
        ...
        if (Version) {
          const permission = await this.getProjectPrivilege(
            Version.project_id.value(),
            account.username
          );
          if (permission) {
            context.success = true;
      ...
      if (key) {
        ...
        if (database) {
          ...
          const systemTables = [
            ...
            {
              table_alias: "privilegegroup",
              keys: ["privilegegroup_id", "id"],
            },
            {
              table_alias: "privilegedetail",
              keys: [
                "privilegegroup_id",
                "id",
                "privilegedetail_id",
            ...
            {
              table_alias: "userprivileges",
              keys: ["id", "username", "privilegegroup_id"],
            },
          ];
          ...
          for (let r = 0; r < serializedImportData.length; r++) {
            ...
            await dbo
              .collection("CACHE_TABLE_NEVER_DIE")
              .deleteOne({ key: `${table_alias}-periods` });
            await dbo
              .collection("CACHE_TABLE_NEVER_DIE")
              .insertOne({ key: `${table_alias}-periods`, value: periods });
          }
          ...
          for (let i = 0; i < tables.length; i++) {
            ...
            for (let j = 0; j < masters.length; j++) {
              ...
              const primary_fields = fields.filter(
              ...
              await dbo
                .collection(`${table_alias}`)
                .createIndex(masterIndexing);
            }
            ...
            await Promise.all(
              foreign_keys.map((key) => {
                ...
                return dbo
                  .collection(`${table_alias}`)
                  .createIndex({ [field.fomular_alias]: 1 });
              })
            );
          ...
          res
            .status(200)
            .send({ success: false, content: "Khum tìm thấy dữ liệu" });
        }
        // }
        // else{
        ...
        res
          .status(200)
          .send({ success: false, content: "Chưa kích hoạt khóa mấy má oi" });
      }
    } else {
  ...
  synchronizeAPIData = async (api) => {
    ...
    if (table) {
      ...
      const oldSumerize = await Database.select(table_alias, {
        position: "sumerize",
      });
  ...
  saveStatisticInfor = async (api) => {
    ...
    if (!table) {
      ...
      for (let i = 0; i < calculates.length; i++) {
        ...
        const oldCal = tableCalculates.find(
          (c) => c.fomular_alias == calculate.fomular_alias
        );
        if (oldCal) {
      ...
      for (let i = 0; i < statistic.length; i++) {
        ...
        const oldCal = tableStatistics.find(
          (c) => c.fomular_alias == statis.fomular_alias
        );
        if (oldCal) {
