...
class ProjectsController extends Controller {
  ...
  get = async (req, res, privileges = ["uad"]) => {
    ...
    if (verified) {
      ...
      if (
        account ||
        decodedToken.username === this.#__defaultAccount.username
      ) {
  ...
  getStatistic = async (req, res) => {
    ...
    if (verified) {
      ...
      for (let i = 0; i < statuses.length; i++) {
        state_statis[i.toString()] = await this.#__projects.count({
          ...
          if (rawProjectId != undefined) {
            ...
            if (isProjectIDValid) {
              ...
              if (project) {
                ...
                context.data.manager = await this.getProjectManager({
                  project_id,
                });
                context.data.members = await this.getProjectMembers({
                  project_id,
                });
                context.data.versions = await this.getProjectVersions({
                  project_id,
                });
                context.data.progress = await this.getProjectProgress({
                  project_id,
                });
  ...
  getProjectManager = async (project) => {
    ...
    const projectManager = await this.#__projectMembers.find({
      project_id,
  ...
  getProjectMembers = async (project) => {
    const { project_id } = project;
    const projectMembers = await this.#__projectMembers.findAll({
      project_id,
      permission: {
        $in: [Controller.permission.spv, Controller.permission.dpr],
      },
    });
  ...
  getProjectProgress = async ({ project_id }) => {
    ...
    if (total > 0) {
      const completed = tasks.filter(
        (task) => task.task_approve == completedStatus
      ).length;
  ...
  createProject = async (req, res, privileges = ["ad"]) => {
    ...
    if (verified) {
      ...
      if (privileges.indexOf(decodedToken.role) != -1) {
        ...
        if (project) {
          const nullCheck = this.notNullCheck(project, [
            "project_name",
            "project_status",
          ]);
          if (project != undefined) {
            if (nullCheck.valid) {
              ...
              const account = await this.#__accounts.find({
                username: create_by,
              });
              if (account) {
                ...
                if (manager) {
                  ...
                  const doesManagerExist = await this.#__accounts.find({
                    username,
                  });
                  if (doesManagerExist) {
                    const Manager = new AccountsRecord(doesManagerExist);
                    if (
                      [Controller.permission.pm].indexOf(
                        Manager.role.value()
                      ) != -1
                    ) {
                      ...
                      if (version) {
                        ...
                        const Version = new VersionsRecord({
                          project_id: Project.project_id.value(),
                          version_name,
                          version_description,
                        ...
                        await this.#__versions.makeDefaultVersion({
                          project_id: Project.project_id.value(),
                          create_by: decodedToken.username,
                        });
                      }
                      ...
                      context.content =
                        "Tạo project & thêm người quản lý dự án thành công!";
                      context.data = await Project.get();
                      ...
                      await this.saveLog(
                        ...
                        req.ip,
                        "__createproject",
                        `__projectname ${Project.project_name.value()} |__manager ${Manager.fullname.value()}`,
                    ...
                    context.content =
                      "Người quản lý dự án này không tồn tại hoặc đã bị xóa";
                  ...
                  await this.saveLog(
                    ...
                    req.ip,
                    "__createproject",
                    `__projectname ${Project.project_name.value()}`,
                    decodedToken.username
                  );
                }
              } else {
  ...
  addMember = async (req, res, privileges = ["uad", "ad", "pm"]) => {
    ...
    const validPrivileges = [
      Controller.permission.spv,
      Controller.permission.dpr,
    ];
    this.writeReq(req);
    ...
    if (verified) {
      ...
      if (
        ...
        if (currentAccount) {
          ...
          if (isProjectIDValid) {
            const project = await this.#__projects.find({
              project_id: parseInt(project_id),
            });
            if (project) {
              const member = await this.#__projectMembers.find({
                project_id: parseInt(project_id),
                username,
              });
              ...
              if (
                (member &&
                  [
                    Controller.permission.mgr,
                    Controller.permission.spv,
                  ].indexOf(member.permission) != -1) ||
                outerPrivileges.indexOf(decodedToken.role) != -1
              ) {
                const failAccounts = [];
                for (let i = 0; i < usernames.length; i++) {
                  ...
                  const account = await this.#__accounts.find({ username });
                  if (account) {
                    const isAMemeber = await this.#__projectMembers.find({
                      project_id: parseInt(project_id),
                      username,
                    });
                    if (!isAMemeber) {
                      if (validPrivileges.indexOf(role) != -1) {
                        ...
                        await this.saveLog(
                          ...
                          `__username ${Member.username.value()}`,
                          decodedToken.username
                        );
                      } else {
                        failAccounts.push({
                          username,
                          reason:
                            "Người dùng này khum thuộc nhóm quyền khả dụng",
                        });
                      }
                    } else {
                      failAccounts.push({
                        username,
                        reason: "Người dùng này đã tồn tại trong dự án",
                      });
                    }
                  } else {
                    failAccounts.push({
                      username,
                      reason: "Người dùng này không tồn tại hoặc đã bị xóa",
                    });
                  }
                }
                if (failAccounts.length > 0) {
                  if (failAccounts.length == usernames.length) {
                    ...
                    context.content =
                      "Khum có người dùng nào được thêm vào dự án vì toàn bộ đều đã bị xóa hoặc khum khả dụng!"; /* passed */
                    context.status = "0x4501064";
                  } else {
                    context.success = true;
                    context.content =
                      "Một vài người dùng không được thêm vào vì họ đã bị xóa hoặc khum khả dụng!"; /* passed */
                ...
                context.content =
                  "Bạn khum có quyền thực hiện thao tác này!"; /* passed */
          ...
          context.content =
            "Account của bạn đã bị xóa hoặc không tồn tại!"; /* passed */
  ...
  update = async (req, res, privileges = ["ad", "pm"]) => {
    ...
    if (verified) {
      ...
      if (
        privileges.indexOf(decodedToken.role) != -1 ||
          ...
          if (account || username === this.#__defaultAccount.username) {
            ...
            if (nullCheck.valid) {
              ...
              if (validatedProjectId) {
                ...
                const oldProject = await this.#__projects.find({
                  project_id: parsedIntProjectId,
                });
                if (oldProject) {
...
| __projectdescription ${
                      oldProject.project_description
                    } => ${Project.project_description.value()} 
| __projectstatus ${
                      oldProject.project_status
                    } => ${Project.project_status.value()}`.replaceAll(
                      "\n",
                      " "
                    ),
                    decodedToken.username
                ...
                context.content =
                  "project_id phải là một số nguyên"; /* passed */
            ...
            if (isProjectIDValid) {
              ...
              if (oldProject) {
                ...
                await this.saveLog(
                  ...
                  req.ip,
                  "__purgeproject",
                  `__projectname ${oldProject.project_name}`,
                  decodedToken.username
                );

                // remove versions
                await this.#__projectMembers.removeMembers(
                ...
                if (user) {
                  ...
                  if (member) {
                    ...
                    await this.saveLog(
                      ...
                      } | __username ${OldManager.username.value()} `,
                      decodedToken.username
                    );
                  }
                  const oldmem = await this.#__projectMembers.find({
                    project_id: project.project_id,
                  ...
                  const Manager = new ProjectMembersRecord({
                    project_id: project.project_id,
                    username: user.username,
                    permission: Controller.permission.mgr,
                  });
                  ...
                  await this.saveLog(
                    ...
                    `__projectname ${
                      project.project_name
                    } | __username ${Manager.username.value()}`,
                    decodedToken.username
                  );
                } else {
  ...
  changeProjectMemberPrivilege = async (
    req,
    res,
    privileges = ["ad", "pm", "ps"]
  ) => {
    ...
    if (verified) {
      ...
      if (thisAccount) {
        ...
        const checkNULL = this.notNullCheck(req.body, [
          "project_id",
          "username",
          "permission",
        ]);
        if (checkNULL.valid) {
          if (thisAccount.username != username) {
            ...
            if (isProjectIDValid) {
              ...
              if (project) {
                const pjmem = await this.#__projectMembers.find({
                  project_id: parsedId,
                  username: thisAccount.username,
                });
                if (pjmem || this.isAdmin(decodedToken)) {
                  if (
                    privileges.indexOf(pjmem.permission) != -1 ||
                    this.isAdmin(decodedToken)
                  ) {
                    const targetAccount = await this.#__accounts.find({
                      username,
                    });
                    if (targetAccount) {
                      const targetPjMem = await this.#__projectMembers.find({
                        project_id: parsedId,
                        username: targetAccount.username,
                      });
                      if (targetPjMem) {
                        const projectPri = this.checkPrivilege(
                          pjmem.permission,
                          targetPjMem.permission
                        );
                        if (projectPri) {
                          const newPriCheck = this.checkPrivilege(
                            pjmem.permission,
                            permission
                          );
                          ...
                          if (newPriCheck) {
                            ...
                            await this.saveLog(
                              ...
                              `__projectname ${
                                project.project_name
                              } | __username ${ProjectMember.username.value()} | __permission __${
                                targetPjMem.permission
                              } => __${ProjectMember.permission.value()} `,
                              decodedToken.username
                            );
                          } else {
                            context.content =
                              "Quyền mới phải nhỏ hơn quyền của ng thực hiện";
                            context.status = "0x4501095";
                          }
                        } else {
                          context.content =
                            "Khum thể thay đổi quyền của ng có quyền lớn hơn mình";
                      ...
                      context.content =
                        "Account chỉ định khum tồn tại hoặc bị xóa";
  ...
  removeMemberFromProject = async (
    ...
    if (verified) {
      ...
      if (thisAccount) {
        const { project_id, username } = req.body;
        const nullCheck = this.notNullCheck(req.body, [
          "project_id",
          "username",
        ]);
        if (nullCheck.valid) {
                  ...
                  if (targetAccount) {
                    if (targetPjMem) {
                      if (
                        ...
                        const ExistedMem = new ProjectMembersRecord(
                          targetPjMem
                        );
                        ...
                        context.content =
                          "Bạn khum thể xóa người dùng có quyền hạn cao hơn mình khỏi dự án";
                        context.status = "0x4501108";
                      }
                    }
                  } else {
                    if (targetPjMem) {
                      const NonExistedMem = new ProjectMembersRecord(
                        targetPjMem
                      );
