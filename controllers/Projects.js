...
class ProjectsController extends Controller {
  ...
  getAllProjectBuOnLyGeneralInfo = async () => {
    const projects = await Database.selectFields("projects", {}, [
      "project_id",
      "project_name",
      "project_code",
      "project_status",
  ...
  projectGeneralCheck = async (req, project_id, privileges = []) => {
    ...
    if (verified) {
      ...
      const projectAndManagerValiator = async () => {
        const validate = await this.validateProjectAndManager(
          project_id,
  ...
  create = async (req, res, privileges = []) => {
    ...
    if (success) {
      ...
      if (nullCheck.valid) {
        ...
        const { decodedToken } = objects;
        const managerAccount = await AccountsModel.find({
        ...
        if (managerAccount) {
          ...
          this.saveLog(
            ...
            req.ip,
            "__createproject",
            `__projectname ${finishedData.project_name} |__manager ${managerAccount?.fullname} | __projectType: ${finishedData.project_type} | __projectProxy: ${finishedData.proxy_server}`,
            decodedToken.username
          );
        } else {
          context.success = false;
        ...
        if (project_id != undefined) {
          ...
          if (project != undefined) {
            ...
            await this.saveLog(
              ...
              req.ip,
              "__purgeproject",
              `__projectname ${project.project_name}`,
              decodedToken.username
            );
    ...
    if (verified) {
      ...
      if (managers.length > 0 && projects.length > 0) {
        ...
        for (let i = 0; i < managers.length; i++) {
          ...
          const projectIn = projects.filter(
            (p) => p.manager?.username == username
          );
    ...
    if (verified) {
      ...
      if (projects) {
        for (let i = 0; i < projects.length; i++) {
          ...
          const stringifyProjectStatus = project_status
            ? project_status.toString()
            : "unknown_status";
          const date = new Date(create_at);
          const year = date.getFullYear();
          const yearExisted = annualStatistic.find(
        ...
        annualStatistic.sort((preYear, nextYear) =>
          preYear.year < nextYear.year ? -1 : 1
        );
      }
    ...
    if (success) {
      ...
      if (this.notNullCheck(req.body, ["project_id", "usernames"]).valid) {
        ...
        if (project) {
          ...
          const queryStart = new Date();
          const users = await AccountsModel.findAll({
          ...
          const members = usernames
            .map(({ username, permission, role }) =>
              Project.createMember(users, username, permission || role)
            )
            .filter((result) => result != undefined);
          this.saveLog(
            "info",
            req.ip,
            "__addmembertoproject",
            `__projectname ${project.project_name}| __username ${members
              .filter((mem) => mem.permission != undefined)
              .map(
                (mem) => `${mem.username}-${mem.fullname}( ${mem.permission} )`
              )
              .join(", ")}`,
            decodedToken.username
          );

          const deployers = members.filter(
            (mem) =>
              mem.permission == Controller.permission.dpr ||
              mem.role == Controller.permission.dpr
          );
          const supervisors = members.filter(
            (mem) =>
              mem.permission == Controller.permission.spv ||
              mem.role == Controller.permission.spv
          );
          ...
          await Project.__modifyAndSaveChange__(
            "deployers",
            serializedDeployers
          );
          await Project.__modifyAndSaveChange__(
            "supervisors",
            serializedSupervisors
          );

          const oldProjectUsernameArray = this.getProjectMembers(project).map(
            (m) => m.username
          );
          const newAddedMembers = usernamesArray.filter(
            (u) => oldProjectUsernameArray.indexOf(u) == -1
          );
          ...
          const deletedUsers = oldProjectUsernameArray.filter(
            (u) => usernamesArray.indexOf(u) == -1
          );
  ...
  removeMember = async (req, res, privileges = []) => {
    ...
    if (success) {
      ...
      if (
        ...
        context.success = false;
        context.content =
          "Khum thể xóa người dùng có quyền cao hơn mình khỏi dự án";
  ...
  changeMemberPrivilege = async (req, res) => {
    ...
    if (success) {
      ...
      if (
        ...
        if (
          this.isAdmin(decodedToken) ||
          this.checkPrivilege(permission, targetPermission)
        ) {
          ...
          context.content =
            "Khum thể thay đổi quyền lớn hơn quyền của người thực hiện";
          context.status = "0x4501095";
          context.success = false;
        }
      } else {
        context.content =
          "Khum thể thay đổi quyền của người dùng có quyền cao hơn người thực hiện";
  ...
  createTaskPeriod = async (req, res) => {
    ...
    if (success) {
      ...
      if (bodyNullCheck.valid) {
        const { Project, decodedToken } = objects;
        const nullCheck = this.notNullCheck(period, [
          "period_name",
          "start",
          "end",
        ]);
        ...
        if (nullCheck.valid) {
          ...
          const doesThisNameExist = periods.find(
            (p) =>
              period.period_name.toLowerCase() == p.period_name.toLowerCase()
          );
          ...
          context.data.period_members = Object.values(
            context.data.period_members
          );
          ...
          this.saveLog(
            ...
            req.ip,
            "__createTaskPeriod",
            `__projectname: ${project.project_name} | __project_code: ${
              project.project_code
            } | __periodname: ${
              period.period_name
            } | __members: ${memberNames.join(", ")}`,
            decodedToken.username
          );
  ...
  updatePeriodApproval = async (req, res) => {
    this.writeReq(req);
    ...
    if (success) {
      ...
      if (oldPeriod) {
        const bodyNullCheck = this.notNullCheck(req.body, ["period"]);
        if (bodyNullCheck.valid) {
          const { period_approve } = period;
          oldPeriod.period_approve = period_approve;
          await Project.__modifyAndSaveChange__(
            `tasks.${period_id}`,
            oldPeriod
          );
      ...
      if (oldPeriod) {
        ...
        if (bodyNullCheck.valid) {
          ...
          this.saveLog(
            ...
            `__projectname: ${project.project_name} | __project_code: ${
              project.project_code
            } | __periodname: ${oldPeriod.period_name} => ${
              newPeriod.period_name
            } | __period_start: ${oldPeriod.start} => ${
              newPeriod.start
            } | __period_end: ${oldPeriod.end} => ${
              newPeriod.end
            } | __member: ${oldMemberNames.join(", ")} => ${newMemberNames.join(
              ", "
            )}`,
            decodedToken.username
          );

          const newAddedMembers = newMembersUsernameList.filter(
            (mem) => originMembers[this.dotEncode(mem)] == undefined
          );
          for (let i = 0; i < newAddedMembers.length; i++) {
            const notify = {
              image_url: decodedToken.avatar,
          ...
          const arraizedOldMembers = Object.values(originMembers).map(
            (u) => u.username
          );
          const deletedUsers = arraizedOldMembers.filter((u) =>
            newMembersUsernameList.indexOf(u) == -1 ? true : false
          );
      ...
      if (tasks) {
        ...
        if (oldPeriod) {
          this.saveLog(
            ...
            req.ip,
            "__removeTaskPeriod",
            `__projectname: ${project.project_name} | __project_code: ${project.project_code} | __periodname: ${oldPeriod.period_name}`,
            decodedToken.username
          );
        }
      }
  ...
  createTask = async (req, res) => {
    ...
    if (success) {
      ...
      if (period) {
        ...
        const startDate = new Date(start);
        const endDate = new Date(end);
        const nullCheck = this.notNullCheck(task, [
          "task_name",
          "task_description",
        ]);
        ...
        if (nullCheck.valid) {
          ...
          this.saveLog(
            ...
            `__projectname: ${project.project_name}| __period_name: ${
              period.period_name
            }| __taskname: ${task.task_name} | __taskdescription: ${
              task.task_description
            } | __taskpriority ${task.task_priority} | __taskmembers: ${users
    ...
    if (success) {
      ...
      if (period) {
        ...
        if (task) {
          ...
          const nullCheck = this.notNullCheck(childTask, [
            "child_task_name",
          ...
          if (nullCheck.valid) {
            ...
            const newChild = await Project.addChildTask(
              period_id,
              task_id,
              childTask
            );
            ...
            this.saveLog(
              ...
              `__projectname: ${project.project_name}| __period_name: ${
                period.period_name
              }| __taskname: ${task.task_name} | __childtaskname: ${
                childTask.child_task_name
              } |  __childtaskdescription: ${
                childTask.child_task_description
              } | __priority ${childTask.priority} | __taskmembers: ${users
                .map((u) => `${u.username}(${u.fullname})`)
                .join(", ")}`,
              decodedToken.username
            );
      ...
      if (period) {
        ...
        if (oldTask) {
          ...
          if (
            ...
            this.isAdmin(decodedToken) ||
            decodedToken.username == create_by.username ||
            true
          ) {
            ...
            switch (type) {
              case "infor":
                ...
                const oldUsernames = Object.values(oldTask.members)
                ...
                newModified = await Project.makeModified(
                  ...
                  this.stringifyObject(
                    { ...oldTask, members: oldUsernames },
                    inforFields
                  ),
                  this.stringifyObject(
                    { ...newTask, members: usernames.join(", ") },
                    inforFields
                  )
                );
                this.saveLog(
                  ...
                  req.ip,
                  "__modifytaskinfor",
                  `__projectname: ${project.project_name}| __period_name: ${period.period_name} | __taskname ${task.task_name} | __taskpriority: ${task.task_priority} => ${newTask.task_priority} |  __taskname ${task.task_name} => ${newTask.task_name} | __taskdescription: ${task.task_description} => ${newTask.task_description}`,
                  decodedToken.username
                );

                const oldMemberUsernames = Object.values(oldTask.members).map(
                  (user) => user.username
                );
                const newMemberUsernames = req.body.task.members;

                const newAddedMembers = newMemberUsernames.filter(
                  (mem) => oldMemberUsernames.indexOf(mem) == -1
                );

                const deletedMembers = oldMemberUsernames.filter(
                  (mem) => newMemberUsernames.indexOf(mem) == -1
                );
              ...
              case "status":
                ...
                this.saveLog(
                  ...
                  `__projectname: ${project.project_name}| __period_name: ${
                    ...
                    task.task_name
                  } | __taskstatus ${Projects.getTaskStatusName(
                    oldStatus
                  )} => ${Projects.getTaskStatusName(newTask.task_status)}`,
                  decodedToken.username
                );
                break;
              ...
              case "approve":
                ...
                this.saveLog(
                  ...
                  `__projectname: ${project.project_name}| __period_name: ${
                    period.period_name
                  } | __taskname ${task.task_name} | ${
                    task.task_approve ? "__approved" : "__unapproved"
                  } => ${newTask.task_approve ? "__approved" : "__unapproved"}`,
                  decodedToken.username
                );
                break;
              default:
            ...
            newTask.task_modified = {
              ...oldTask.task_modified,
              ...newModified,
            };
        ...
        if (task) {
          ...
          if (
            ...
            this.saveLog(
              ...
              req.ip,
              "__purgetask",
              `__projectname: ${project.project_name}| __period_name: ${period.period_name} |  __taskname ${task.task_name}`,
              decodedToken.username
            );
          } else {
            context.content = "Khum có quyền thực hiện thao tác này";
        ...
        if (task) {
          ...
          if (oldChildTask) {
            ...
            if (
              ...
              newTask.members = serializedUsers;
              period.tasks[`${task_id}`].child_tasks[`${child_task_id}`] =
                newTask;

              const oldUsernames = Object.values(oldChildTask.members)
                .map((user) => user.fullname)
                .join(", ");
              ...
              const newModified = await Project.makeModified(
                ...
                decodedToken,
                this.stringifyObject(
                  { ...oldChildTask, members: oldUsernames },
                  infofields
                ),
                this.stringifyObject(
                  {
                    ...newTask,
                    members: Object.values(serializedUsers)
                      .map((u) => u.fullname)
                      .join(", "),
                  },
                  infofields
              ...
              const oldMemberUsernames = Object.values(
                oldChildTask.members
  ...
  isProjectMatched = (project, criteria) => {
    ...
    const corespondingStatusID =
      this.translateCriteriaToStatusIfAvailable(criteria);
    ...
    if (
      (project_status && project_status.toString() == criteria) ||
      project_status == corespondingStatusID
    ) {
      valid = true;
    }
    ...
    if (success) {
      if (criteria) {
        ...
        const matchedProjects = projects.filter((project) =>
          this.isProjectMatched(project, criteria)
        );
