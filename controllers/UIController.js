...
class UIController extends Controller {
  ...
  fieldsAndKeysSeperator = (table) => {
    ...
    const primaryFields = parsedFields.filter(
      (field) => primary_key.indexOf(field.id) != -1
    );
    const normalFields = parsedFields.filter(
      (field) => primary_key.indexOf(field.id) == -1
    );
      ...
      if (bodyNullCheck.valid) {
        ...
        if (uiNullCheck.valid) {
          ...
          if (!existedURL) {
            ...
            if (table) {
              const { keyFields, normalFields } =
                this.fieldsAndKeysSeperator(table);
              ...
              if (keyFields.length > 0) {
                if (normalFields.length != 0) {
                  ...
                  this.saveLog(
                    ...
                    req.ip,
                    "__createUI",
                    `__projectname: ${project.project_name} | __versionname ${version.version_name} | __uititle: ${newUi.title} __uiurl:  ${newUi.url}`,
                    user.username
                  );
                } else {
                  context.content =
                    "Bảng không có trường nào khác ngoài khóa chính";
    ...
    if (success) {
      ...
      if (nullCheck.valid) {
        ...
        if (!existedURL) {
          ...
          if (table) {
            const atLeastOneSideTableDoesNotExist = uiTables.find(
              (tb) => tables[`${tb}`] == undefined
            );
    ...
    if (success) {
      ...
      if (version.uis[`${ui_id}`]) {
        ...
        this.saveLog(
          ...
          req.ip,
          "__updateUIstatus",
          `__projectname: ${project.project_name} | __versionname ${version.version_name} | __uititle: ${ui.title} __uiurl: ${ui.url} | __uistatus: ${ui.status} => ${status}`,
          user.username
        );
      } else {
  ...
  removeUI = async (req, res) => {
    ...
    if (success) {
      ...
      const corespondingAPIS = arraizedAPIs.filter(
        (api) => toBeRemovedAPIs.indexOf(api.url) != -1
      );
      ...
      this.saveLog(
        ...
        req.ip,
        "__removeUI",
        `__projectname: ${project.project_name} | __versionname ${version.version_name} | __uititle: ${ui.title} __uiurl: ${ui.url}`,
        user.username
      );
    }
        ...
        if (uiNullCheck.valid) {
          ...
          if (!existedURL) {
            ...
            if (api.api_method == "get") {
              const view = await Project.createAPIView(
                ui.title,
                layout_id,
                api,
                URL,
  ...
  updateChildComponent = (components, target_id, values) => {
    for (let i = 0; i < components.length; i++) {
      ...
      if (id == target_id) {
        ...
        if (children) {
          components[i].children = this.updateChildComponent(
  ...
  createApiSetOnTable = (table, version) => {
    ...
    const normalFields = source.fields.filter(
      (f) => paramsId.indexOf(f.id) == -1
    );

    const fieldsWithoutParams = bodyId.filter(
      (field) => paramsId.indexOf(field) == -1
    );
    ...
    if (buttons.approve.state) {
      const approveTable = source.tables.find(
        (tb) => tb.id == buttons.approve.field.table_id
      );
      if (approveTable) {
        const APPROVE = {
    ...
    if (buttons.unapprove.state) {
      const unapproveTable = source.tables.find(
        (tb) => tb.id == buttons.unapprove.field.table_id
      );
      if (unapproveTable) {
        const UNAPPROVE = {
  ...
  mapApiToUIRecursive = (pages, target_id, page) => {
    for (let i = 0; i < pages.length; i++) {
      ...
      if (page_id == target_id) {
        ...
        if (children) {
          pages[i].children = this.mapApiToUIRecursive(
            pages[i].children,
            target_id,
            page
          );
      ...
      for (let i = 0; i < flattenPages.length; i++) {
        ...
        for (let j = 0; j < flattenComponents.length; j++) {
          ...
          if (name == "table" && props && props.source.tables.length > 0) {
            ...
            page.component = this.updateChildComponent(
              page.component,
              cpn.id,
              props
            );
          ...
          if (name == "custom_button") {
            ...
            const foreignPrimaryFields = foreignPrimaryKey.map(
              (key) => foreignTable.fields[`${key}`]
            );
          ...
          if (name == "inline_statis") {
            ...
            if (foreignTable) {
              ...
              const criterias = `"${group_by.fomular_alias}" == "${
                value[primary_field.fomular_alias]
              }"`;
