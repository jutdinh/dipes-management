...
class ActivationController extends Controller {
  ...
  get = async (req, res) => {
    ...
    for (let i = 0; i < projects.length; i++) {
      ...
      const minimizedProject = {
        ...project,
        create_by: `${fullname}(${username})`,
      };
  ...
  generateActivationKey = async (req, res) => {
    ...
    if (success) {
      ...
      if (this.validMac(targetMachine)) {
        ...
        const rawProject = await ProjectsModel.find({
        ...
        if (rawProject) {
          ...
          const isKeyExisted = keys.find(
            (key) => key.MAC_ADDRESS == targetMachine
          );

          if (!isKeyExisted) {
            const key = Projects.generateActivationKey(
              targetMachine,
              project_id
            );
  ...
  getMachineID = async () => {
    const machineId = await new Promise((resolve, reject) => {
      exec(
        `echo '${process.env.LINUX_PWD}' | sudo -S dmidecode -s system-uuid`,
        (err, stdout, stderr) => {
  ...
  validateKey = (key) => {
    const { header, bodyLength, footer } = Projects.validator;
