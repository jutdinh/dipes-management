...
class TablesController extends Controller {
  ...
  createTable = async (req, res) => {
    ...
    if (success) {
      ...
      this.saveLog(
        ...
        req.ip,
        "__createtable",
        `__projectname ${project.project_name} | __versionname ${version.version_name} | __tablename: ${tb.table_name}`,
        user.username
      );
    }
    delete context.objects;
  ...
  deleteTable = async (req, res) => {
    ...
    if (success) {
      ...
      this.saveLog(
        ...
        req.ip,
        "__removetable",
        `__projectname ${project.project_name} | __versionname ${version.version_name} | __tablename: ${table.table_name}`,
        user.username
      );
    }
    ...
    if (success) {
      ...
      if (table) {
        ...
        for (let i = 0; i < fields.length; i++) {
          ...
          if (
            newFields[`${field.id}`] != undefined &&
            !this.isForeignKey(table, field.id)
          ) {
            field.props = { ...field };
        ...
        serializedFields.map((field) => {
          const newField = fields.find(
            (f) => f.id == field.id && f.id != undefined
          );
        ...
        for (let i = 0; i < arraizedNewFields.length; i++) {
          ...
          if (!field) {
            ...
            this.saveLog(
              ...
              req.ip,
              "__createField",
              `__projectname ${project.project_name} | __versionname: ${version.version_name} | __tablename: ${table.table_name} | __fieldname: ${corespondingField.field_name} | __datatype: ${corespondingField.props.DATATYPE}`,
              user.username
            );
          } else {
            const areTheseFieldsEqual = objectComparator(
              field,
              corespondingField
            );
            if (!areTheseFieldsEqual) {
              ...
              this.saveLog(
                ...
                req.ip,
                "__modifyField",
                `__projectname ${project.project_name} | __versionname: ${version.version_name} | __tablename: ${table.table_name} | __fieldname: ${field.field_name} => ${corespondingField.field_name}  | __datatype: ${field.props.DATATYPE} => ${corespondingField.props.DATATYPE} | NULL ${field.props.NULL} => ${corespondingField.props.NULL} | LENGTH ${field.props.LENGTH} => ${corespondingField.props.LENGTH} | AUTO_INCREMENT ${field.props.AUTO_INCREMENT} => ${corespondingField.props.AUTO_INCREMENT} | MIN ${field.props.MIN} => ${corespondingField.props.MIN} | MAX ${field.props.MAX} => ${corespondingField.props.MAX} | FORMAT ${field.props.FORMAT} => ${corespondingField.props.FORMAT} | DECIMAL_PLACE ${field.props.DECIMAL_PLACE} => ${corespondingField.props.DECIMAL_PLACE} | DEFAULT ${field.props.DEFAULT} => ${corespondingField.props.DEFAULT} | DEFAULT_TRUE ${field.props.DEFAULT_TRUE} => ${corespondingField.props.DEFAULT_TRUE} | DEFAULT_FALSE ${field.props.DEFAULT_FALSE} => ${corespondingField.props.DEFAULT_FALSE}`,
                user.username
              );
            } else {
              /**
        ...
        uis.map((ui) => {
          ...
          const tableComponent = components.find(
            (cpn) => cpn.table_id == updatedTable.id
          );
          if (tableComponent) {
            const keys = Object.keys(tableComponent).filter((key) =>
              key.includes("api_")
            );
  ...
  modifySlave = (Project, version, field) => {
    ...
    if (tables && tables.length > 0) {
      for (let i = 0; i < tables.length; i++) {
        ...
        const key = foreign_keys.find(
          (fk) => fk.ref_field_id.toString() == id.toString()
        );
  ...
  removeSlaveRelation = (Project, version, master_id) => {
    ...
    for (let i = 0; i < tables.length; i++) {
      const table = tables[i];
      const foreign_keys = Object.values(table.foreign_keys);
      const newForeignKey = foreign_keys.filter(
        (key) => key.ref_field_id.toString() != master_id.toString()
      );
      ...
      if (table) {
        if (primary_key && Array.isArray(primary_key)) {
          ...
          for (let i = 0; i < primary_key.length; i++) {
            ...
            if (!field) {
              ...
              if (
                Projects.validPrimaryTypes.indexOf(field.props.DATATYPE) == -1
              ) {
                valid = false;
          ...
          if (valid) {
            ...
            const purgedPrimaryKey = oldPrimaryKey.filter(
              (key) => primary_key.indexOf(key) == -1
            );
            ...
            context.primary_key = {
              success: true,
              content: "Cập nhật khóa chính thành công",
              status: "0x",
            };
            this.saveLog(
              "warn",
            ...
            for (let i = 0; i < apis.length; i++) {
              ...
              if (changedAPIs.indexOf(url) != -1) {
                ...
                if (api_method == "put") {
                  api.params = table.primary_key;
                  api.body = newfields
                    .filter((f) => table.primary_key.indexOf(f.id) == -1)
                    .map((field) => field.id);
                }
            ...
            context.primary_key = {
              success: false,
              content:
                "Có vài trường hoặc bị bắt cóc, hoặc có thể bị rỗng, hoặc có kiểu khum đúng",
              status: "0x",
            };
          }
        } else {
          ...
          context.primary_key = {
            success: false,
            content: "Khóa chính khum tồn tại hoặc sai kiểu",
            status: "0x",
          };
        }
        ...
        if (foreign_keys && Array.isArray(foreign_keys)) {
          ...
          for (let i = 0; i < foreign_keys.length; i++) {
            const { field_id, table_id, ref_field_id, cascade } =
              foreign_keys[i];
          ...
          if (valid) {
            ...
            context.foreign_keys = {
              success: true,
              content: "Cập nhật khóa ngoại thành công",
              status: "0x",
            };
            this.saveLog(
              "warn",
            ...
            context.foreign_keys = {
              success: false,
              content: "Ít nhất có một khóa ngoại không hợp lệ",
              status: "0x",
            };
          }
        } else {
          ...
          context.foreign_keys = {
            success: false,
            content: "Khóa ngoại không tồn tại hoặc sai kiểu",
            status: "0x",
          };
        }
  ...
  updateTable = async (req, res) => {
    ...
    const display_fields = req.body.display_fields
      ? req.body.display_fields
      : [];
    ...
    if (success) {
      ...
      if (table) {
        ...
        for (let i = 0; i < field_ids.length; i++) {
          ...
          table.foreign_keys = currentFKs.filter(
          ...
          if (primary_key.indexOf(field_id) != -1) {
            ...
            table.primary_key = table.primary_key.filter(
              (key) => key != field_id
            );
          }
          ...
          for (let j = 0; j < apis.length; j++) {
            ...
            if (changedAPIs.indexOf(api.url) != -1) {
              ...
              if (["get"].indexOf(api.api_method) != -1) {
                ...
                const newApiFields = arraizedFields.filter(
          ...
          this.saveLog(
            ...
            req.ip,
            "__removeField",
            `__projectname: ${Project.getData().project_name}| __versionname: ${
              version.version_name
            } | __tablename: ${table.table_name} | __fieldfomularalias: ${
              field.fomular_alias
            } | __fieldname: ${field.field_name}`,
            user.username
          );
        }
