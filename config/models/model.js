...
const { Field, Number, String, Datetime, Int, Bool, Enum, List, Serialized } = require('./fields');
const { 
    successLog,
    errorLog,
    warningLog,
...
class Model {
    ...
    constructor( modelName = undefined ){        
        ...
        if( modelName ){            
            this.__addProperty__( "id", Model.types.int, { required: false, auto: true } );
        }
    }
    ...
    __addPrimaryKey__ = ( fieldNames ) => {
        ...
        if(typeof(fieldNames) == "string"){
            ...
            if( Array.isArray( fieldNames ) ){
                ...
                errorLog("Thêm khóa chính thất bại: Kiểu dữ liệu không hợp lệ, khóa chính phải có kiểu <String> || <String>[]")
            }
        }
        if( fields.length > 0 ){
            for( let i = 0 ; i < fields.length; i++ ){
                ...
                if( this.__doesPropertyExist__(field) ){
                    if( !this.__isFieldPrimary__(field) ){
                        ...
                        infoLog(`Thêm khóa chính thành công: ${ this.__getName__() }(${ oldPrimaryKey.join(', ') }) => ${ this.__getName__() }(${ this.#__primaryKey.join(', ') })`)
                    }else{
                        warningLog(`Thêm khóa chính thất bại: Trường ${ field } đã là khóa chính`)
                    }
                }else{
            ...
            if( datatype ){            
                if( datatype == "model"  ){
                    ...
                    for( let j = 0 ;  j < children.length; j++ ){
                        ...
                        properties[ Property.__fieldName ][child] = this.__getDataRecursive__( Property, data[Property.__fieldName][child], child )
                    }
                }else{
                    if(  datatype == "json" ){
                        const childrenData = data[ Property.__fieldName ] 
                        const children = Object.keys(childrenData ? childrenData:  {})
                        properties[ Property.__fieldName ] = this.__getDataRecursive__( Property, data[Property.__fieldName] )                        
                    }else{                                            
                        Property.value(data[Property.__fieldName]) /* Auto validation */
                        if( Property.__datatype == "datetime" ){
                            properties[ Property.__fieldName ] = data[Property.__fieldName] ? Property.getFormatedValue() : Property.__default 
                        }else{
                            properties[ Property.__fieldName ] = data[Property.__fieldName] != undefined ? this.__dotDecode__(Property.value()) : Property.__default                        
                        }
                    }                            
                }
            }
    ...
    __getNameRecursive__ = ( thisObject ) => {
        if( thisObject.__parent != undefined ){
            if( thisObject.__parent.__datatype == "model" ){               
                return this.__getNameRecursive__( thisObject.__parent ) + `.${ thisObject.__modelName ? thisObject.__modelName : thisObject.__fieldName }`
            }else{
                return this.__getNameRecursive__( thisObject.__parent ) + `.${ thisObject.__fieldName }`
            }
        }else{
    ...
    __addProperty__ = (propertyName, dataTypeObject, dataTypeProperties = undefined) => {
        ...
        if( !this.__doesPropertyExist__( propertyName ) ){
            if( type == "model" ){
                ...
                infoLog(`${ this.__getName__() } => { ...${ this.__getName__() }, ${ propertyName }: <MODEL> }`)
            }else{
                const fieldObject = new dataTypeObject(propertyName, undefined, dataTypeProperties)
                this[`${propertyName}`] = fieldObject
                infoLog(`${ this.__getName__() } => { ...${ this.__getName__() }, ${ propertyName }: <${ fieldObject.__datatype }> }`)
            }    
            const parent = {...this};                        
        ...
        for( let i = 0; i < keys.length; i++ ){
            ...
            if( datatype ){            
                if( datatype == "model"  ){
                    ...
                    for( let j = 0 ;  j < children.length; j++ ){
                        const child = children[j]                        
                        properties[ Property.__fieldName ][child] = this.__recursiveTreeWithData__( Property, data[Property.__fieldName][child], child )
                    }
                }else{
                    if(  datatype == "json" ){
                        const childrenData = data[ Property.__fieldName ]                                             
                        ...
                        Property.value(data[Property.__fieldName]) /* Auto validation */
                        properties[ Property.__fieldName ] = data[Property.__fieldName] != undefined ? this.__dotDecode__(data[Property.__fieldName]) : Property.__default                        
        ...
        for( let i = 0; i < keys.length; i++ ){
            ...
            if( datatype ){            
                if( datatype == "model" ){
                    ...
                    for( let j = 0 ;  j < children.length; j++ ){
                        const child = children[j]                        
                        properties[ Property.__fieldName ][child] = this.__recursiveTreeToFulfillData__( Property, data[Property.__fieldName][child] )
                    }
                }else{  
                    if( datatype == "json" ){                       
                        properties[ Property.__fieldName ] = this.__recursiveTreeToFulfillData__( Property, data[Property.__fieldName])                                   
                    }else{   
                                       
                        properties[ Property.__fieldName ] = this.__dotEncode__(data[Property.__fieldName])
                    }
    ...
    __modifyChildrenRecursive__ = (parent, paths, data)  => {        
        if( paths.length > 1 ){
            if( parent[ paths[0] ] != undefined ){
                parent[ paths[0] ] =  this.__modifyChildrenRecursive__( parent[ paths[0] ], paths.splice( 1 ), data )             
            }            
        }else{
            parent[ paths[0] ] = data 
        }
    ...
    __modifyChildren__ = ( path, value, force = false ) => {
        if( force ){
            ...
            const newData = this.__modifyChildrenRecursive__( {...data }, splittedPath, value )
    ...
    __modifyAndSaveChange__ = async ( path, value ) => {
        ...
        if( data.id != undefined ){
            ...
            errorLog("Không thể cập nhật vì đối tượng không tồn tại trong cơ sở dữ liệu")
        }
    }
            ...
            if( typeof(child) == "object" ){
                ...
                if( child instanceof Date ){
                    ...
                    if( !Array.isArray(child) ){
                        if( parentPath.length > 0 ){
                            paths.push(...this.__listAllPath__( child, `${parentPath}.${key}` ))
                        }else{
        ...
        for( let i = 0; i < keys.length; i++ ){
            ...
            if( datatype ){            
                if( datatype == "model" ){
                    ...
                    for( let j = 0 ;  j < children.length; j++ ){
                        const child = children[j]                        
                        properties[ Property.__fieldName ][child] = await this.__fillDataFromPropertiesRecursive__( Property, data[Property.__fieldName][child] )
                    }
                }else{  
                    if( datatype == "json" ){                       
                        properties[ Property.__fieldName ] = await this.__fillDataFromPropertiesRecursive__( Property, data[Property.__fieldName])                                   
                    }else{                
                        if( Property.__auto ){
                            if( data[Property.__fieldName] == undefined ){
                                properties[ Property.__fieldName ] = await this.#__database.__getNewId__()
                            }else{
                        ...
                        if( Property.__required &&  data[Property.__fieldName] == undefined){
